...
export default function GitPage() {
  ...
  useEffect(() => {
    if (loading) return;
    fetch("/api/download/project-zip?github=1&action=status")
            ...
            <div className="flex gap-2">
              {githubConnection?.connected ? (
                <Button
                  ...
                  onClick={async () => {
                    const response = await fetch("/api/download/project-zip?github=1&action=disconnect", { method: "POST" });
                ...
                <Button
                  ...
                  size="sm"
                  className="bg-white text-black hover:bg-gray-100"
                  onClick={() => window.location.assign("/api/download/project-zip?github=1")}
                >
