...
export default function GitPage() {
  ...
  useEffect(() => {
    if (loading) return;
    fetch("/api/nova-github?action=status")
            ...
            <div className="flex gap-2">
              {githubConnection?.connected ? (
                <Button
                  ...
                  onClick={async () => {
                    const response = await fetch("/api/nova-github?action=disconnect", { method: "POST" });
                ...
                <Button
                  ...
                  size="sm"
                  className="bg-white text-black hover:bg-gray-100"
                  onClick={() => window.location.assign("/api/nova-github")}
                >
