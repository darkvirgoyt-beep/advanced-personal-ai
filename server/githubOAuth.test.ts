...
describe("GitHub OAuth workspace isolation", () => {
  ...
  it("returns connection status only for the workspace that owns the connection", async () => {
    ...
    const alphaResponse = await api("/api/nova/github/status", alpha);
    const betaResponse = await api("/api/nova/github/status", beta);
  ...
  it("disconnects only the current workspace while preserving other workspaces", async () => {
    ...
    const disconnectResponse = await api("/api/nova/github/disconnect", alpha, { method: "POST" });
  ...
  it("links a successful callback to the originating anonymous workspace and exposes the linked login", async () => {
    ...
    try {
      const callbackResponse = await api("/api/nova/github/callback?code=real-code&state=pending-alpha", alpha);
    ...
    const statusResponse = await api("/api/nova/github/status", alpha);
